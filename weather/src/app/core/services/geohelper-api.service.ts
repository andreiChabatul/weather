      ...
      .subscribe((v) =>
        v.result
          .filter(
            (city) =>
              city.localityType &&
              (city.localityType.name === 'город' ||
                city.localityType.name === 'city')
          )
          .map((city) => this.store.dispatch(new AddSearchCity(city.name)))
      );
  }

  getCity(value: string, pagination: number): Observable<IApiGeohelper> {
    return this.http.get<IApiGeohelper>(URL_GEOHELPER, {
      params: {
        'locale[lang]': 'ru',
        'filter[name]': value,
