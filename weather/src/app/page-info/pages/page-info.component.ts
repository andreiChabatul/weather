      ...
      .pipe(
        ...
        switchMap((coordinate) => {
          if (coordinate.length) {
            this.openweathermapApiService
              .getWeather<IOpenweathermapForecastFive>(
                coordinate[0],
                'forecast'
              )
              .subscribe((value) =>
                this.store.dispatch(new AddItemForecast(value))
              );
            return this.openweathermapApiService.getWeather<IOpenweathermap>(
              coordinate[0],
              'weather'
            );
          } else {
