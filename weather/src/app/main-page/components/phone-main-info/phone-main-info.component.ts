...
import {
  ICoordinate,
  IOpenweathermap,
} from 'src/app/store/models/openweathermap';
...
export class PhoneMainInfoComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges(): void {
    this.weatherInfo$ = this.openweathermapApiService
      .getWeather<IOpenweathermap>(this.coor, 'weather')
      .subscribe((info) => (this.weatherInfo = info));
    this.weatherInfoForecast$ = this.openweathermapApiService
      .getWeather<IOpenweathermapForecastFive>(this.coor, 'forecast')
      .subscribe((info) => {
        this.weatherInfoDate = info.list.filter(
  ...
  clickDate(index: number): void {
    ...
    this.transformVisible = `transform: translateX(${
      -this.activeIndex * 30
    }px)`;
  }
